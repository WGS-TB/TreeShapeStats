...
main = function(low,high,nstat){
  ...
  resolutionTest <- matrix (0, nrow = high -low +1 , ncol= nstat)
  for (tip in (low:high)) {
    ...
    for (s in (1:nstat)){
      result <-  (-0.5) * ((t(NSt[,s]) %*% Ds) %*% NSt[,s])
     
      
      result=as.numeric(result)
      # scale the result
      resolutionTest[tip-(low-1),s] <- (result - values[n])/ dist 
      cat(", ", file = filename, append=TRUE)
      cat(resolutionTest[tip-(low-1),s], file = filename, append=TRUE)
    }
    cat("\n" , file = filename, append=TRUE)
  } 
